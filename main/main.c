...
/* A provider layout/type mismatch means this firmware can no longer safely
 * interpret the persisted sensor registry.  The registry shares NVS with
 * the device configuration, so clear the whole partition and reboot into a
 * genuinely unconfigured state. */
static void erase_nvs_after_incompatible_sensor_config(void) {
  ESP_LOGW(TAG,
           "Stored sensor configuration is incompatible; erasing NVS and "
           "restarting");
  ESP_ERROR_CHECK(nvs_flash_erase());
  esp_restart();
}
...
void app_main(void) {
  ...
  /* Restore the complete sensor registry. With no stored registry, bootstrap
   * one persisted numeric-name instance from every provider's defaults. */
  device_config_load_from_nvs();
  const esp_err_t registry_err = registry_init_on_boot();
  if (registry_err == ESP_ERR_INVALID_STATE) {
    erase_nvs_after_incompatible_sensor_config();
  }
